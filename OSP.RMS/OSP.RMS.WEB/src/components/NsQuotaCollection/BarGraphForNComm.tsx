...
export interface IBarPerMonthNComm {
  Months: string;
  "NCom 30 days": number;
  "NCom 60 days": number;
  "NCom 90 days": number;
...
interface Props {
  data: IBarPerMonthNComm[];
}

const BarGraphForNComm = ({ data }: Props) => {
  ...
  const sumOfQ30 = data.reduce((accu, data) => {
    return accu + data["NCom 30 days"];
  }, 0);

  const sumofQ60 = data.reduce((accu, data) => {
    return accu + data["NCom 60 days"];
  }, 0);

  const sumofQ90 = data.reduce((accu, data) => {
    return accu + data["NCom 90 days"];
  }, 0);
        ...
        <Box w="100%" p={5} bgColor="white" borderRadius={10} shadow="lg">
          ...
          <Box pr={5} pt={10}>
            ...
            >
              ...
              <Bar dataKey="NCom 30 days" fill="#8884d8" />
              <Bar dataKey="NCom 60 days" fill="#FFBB28" />
              <Bar dataKey="NCom 90 days" fill="#82ca9d" />
            </BarChart>
          </Box>
        </Box>
        {/* </Box> */}
          ...
          <TableContainer>
            <Table>
              <Thead h="16">
                <Tr>
                  <Th>MONTH</Th>
                  <Th isNumeric>NCom QUOTA 30 DAYS</Th>
                  <Th isNumeric>NCom QUOTA 60 DAYS</Th>
                  <Th isNumeric>NCom QUOTA 90 DAYS</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data.map((item) => (
                  <Tr key={item.Months}>
                    <Td>{item.Months}</Td>
                    <Td isNumeric>
                      {item["NCom 30 days"].toLocaleString(undefined, {
                        maximumFractionDigits: 3,
                      })}
                    </Td>
                    <Td isNumeric>
                      {item["NCom 60 days"].toLocaleString(undefined, {
                        maximumFractionDigits: 2,
                      })}
                    </Td>
                    <Td isNumeric>
                      {item["NCom 90 days"].toLocaleString(undefined, {
                        maximumFractionDigits: 2,
                      })}
                    </Td>
                  </Tr>
...
export default BarGraphForNComm;
